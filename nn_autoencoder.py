...
def nn_autoencoder(train_dataset, test_dataset, threshold):
	...
	encode = BatchNormalization()(encode)
	encode = Dense(1, activation='relu',kernel_regularizer=regularizers.l1(0.01))(encode)
	encode = BatchNormalization()(encode)
	...
	acc_ax.plot(history.history['accuracy'], 'g', label='Train acc', linewidth=2)
	...
	scored['Threshold'] =  scored.quantile(threshold)[0]
